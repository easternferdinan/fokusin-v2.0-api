...
from services.tasks_service import get_deadline_urgent_tasks_service, get_high_priority_tasks_service, get_incomplete_tasks_service
...
def get_dashboard_data_service(db: Session, user_id: UUID4) -> DashboardResponse:
    ...
    deadline_urgent_tasks_count = get_deadline_urgent_tasks_service(db, user_id, count_only=True)
    today_pomodoro_minutes = get_today_pomodoro_minutes_service(db, user_id)
    deadline_urgent_tasks = get_deadline_urgent_tasks_service(db, user_id)
    ...
    return DashboardResponse(
        ...
        deadline_urgent_tasks_count=deadline_urgent_tasks_count,
        today_pomodoro_minutes=today_pomodoro_minutes,
        deadline_urgent_tasks=deadline_urgent_tasks
    )