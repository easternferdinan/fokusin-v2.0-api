...
def get_sleep_quality_service(db: Session, user_id: uuid.UUID, past_days_count: int = 7) -> List[int]:
    """
    Retrieve sleep quality for a specific user within a given number of past days.

    Args:
        past_days_count (int): Number of past days to retrieve sleep quality for (default: 7)

    Returns:
        List[int]: List of sleep quality (int) for a specific user within a given number of past days
    """
    try:
        return [row[0] for row in db.query(StressAnalysis.sleep_quality).filter(
            StressAnalysis.user_id == user_id,
            StressAnalysis.created_at.cast(Date) >= datetime.now(UTC).date() - timedelta(days=past_days_count)
        ).order_by(StressAnalysis.created_at.desc()).all()]
    except SQLAlchemyError as e:
        raise DatabaseOperationError("Failed to retrieve sleep quality") from e
